...
const Stats = ({ remainingTime, setRemainingTime, newDay, setNewDay }) => {
  ...
  useEffect(() => {
    if (!pendingBalanceIsLoading && !balanceIsLoading && balance[0]) {
      if (pendingBalance.length == 0) {
        ...
        setMyBalance(
          Moralis.Units.FromWei(pendingBalance[0].attributes.balance)
        );
      }
    }
  });
  ...
  const deposit = () => {
    fetch().then(() => {
      let newAllocation =
        Number(user.get("allocation")) + Number(depositAmount);
      setUserData({ allocation: newAllocation });
      let newTotalAllocation =
        Number(dailyStats[0].attributes.totalAllocation) +
  ...
  const {
    ...
    "EthBalance",
    (query) => query.equalTo("address", user.get("ethAddress")),
    [],
    { live: true }
  );

  const { data: pendingBalance, isLoading: pendingBalanceIsLoading } =
    useMoralisQuery(
      "EthBalancePending",
      (query) =>
        query
  ...
  const { data: dailyStats, isLoading: dailyStatsIsLoading } = useMoralisQuery(
    "DailyStats",
    (query) => query.descending("date").limit(2),
    [],
    { live: true }
  );
        ...
        <div className="col col-md-3 order-2 order-md-1">
          <div className="card h-100">
            <div className="card-body p-2 text-center">
              <span className="card-text fw-light">
              ...
              <h5 className="card-title">
                {user.get("lastPayout").toFixed(4) + " ROP"}
              </h5>
              <span className="card-text fw-light">{`My Initial Deposit`}</span>
              <h5 className="card-title">
                {user.get("initialDeposit").toFixed(4) + " ROP"}
              </h5>
            </div>
          </div>
        </div>
        <div className="col col-md-6 order-1 order-md-2">
          <div className="card bg-dark text-light border-danger rounded-pill">
            <div className="card-body p-2 text-center justify-content-center">
              <span className="card-text fw-light d-block lead">
                {"Stake Now"}
              </span>
              <div className="input-group mb-2 w-75 mx-auto">
                ...
                <input
                  type="number"
                  className={`form-control bg-dark border-secondary text-light ${styles.depositInput}`}
                  placeholder="0"
                  min="0"
                  inputMode="numeric"
                  onChange={(e) => depositAmountValidation(e)}
                ></input>
                <button
                  className="btn btn-outline-secondary"
                  type="button"
                  id="button-addon2"
                  onClick={(e) => maxClicked(e)}
                >
                  Max
                </button>
              </div>
              <div className="row justify-content-center px-5">
                <div className="col justify-content-center">
                  <button
                    className="btn btn-danger text-dark gradientButton"
                    onClick={() => deposit()}
                  >
                    {"Deposit"}
                  </button>
                  <span className="card-text fw-light d-block">
                    {"Wallet Balance"}
                  </span>
                  <h5 className="card-title">
                    {myBalance.toFixed(4) + " ROP"}
                  </h5>
                </div>
                <div className="col justify-content-center">
                  ...
                  <h5 className="card-title">
                    {user.get("allocation").toFixed(4) + " ROP"}
                  </h5>
                </div>
              </div>
            </div>
        ...
        <div className="col col-md-3 order-3 order-md-3">
          <div className="card h-100">
            <div className="card-body p-2 text-center">
              ...
              <h5 className="card-title">
                {dailyStats[0].attributes.totalAllocation.toFixed(4) + " ROP"}
              </h5>
              <span className="card-text fw-light">
                {"Total Value of Latest Payout"}
              </span>
              <h5 className="card-title">
                {dailyStats[1].attributes.totalDistribution.toFixed(4) + " ROP"}
              </h5>
            </div>
          </div>
        </div>
